...
print("=" * 60)
print("RecBole AutoML with Ray Tune")
print("=" * 60)
print("✅ 라이브러리 로드 완료\n")
...
base_config = {
    ...
    # Mixed Precision 비활성화: LightGCN의 sparse 연산은 FP16 미지원
...
def train_recbole(config_params):
    """Ray Tune trainable 함수 - RecBole 모델 학습"""
...
tuner = tune.Tuner(
    ...
    run_config=RunConfig(
        name=f'recbole_lightgcn_automl_{pd.Timestamp.now().strftime("%Y%m%d_%H%M%S")}',
...
params_filename = f"{params_output_dir}/best_hyperparams_lightgcn_{t_params.year}{t_params.month:02d}{t_params.day:02d}{t_params.hour:02d}{t_params.minute:02d}{t_params.second:02d}.json"
...
# ============================================================
print("\n" + "=" * 60)
print("Ray Tune AutoML 최적화 결과 요약")
print("=" * 60)
...
print(f"   Ray Tune 결과: {ray_results_path}/recbole_lightgcn_automl/")

print("\n📊 상위 5개 Trial 결과:")
top5 = results_df.nlargest(5, 'recall@5')[['config/embedding_size', 'config/n_layers',
                                              'config/reg_weight', 'config/learning_rate',
                                              'recall@5', 'ndcg@5']]
