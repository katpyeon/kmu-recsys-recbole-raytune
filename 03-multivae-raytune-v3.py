#!/usr/bin/env python3
"""
RecBole AutoML - MultiVAE Hyperparameter Optimization (Phase 1.5 v3)

Phase 1.5 개선사항 (보수적 접근 - 단순한 백화점 데이터):
v2 실패 원인:
- Learning rate [0.001, 0.03]로 변경 → 최적값 0.000517 제외! ❌
- Dropout [0.45, 0.65]로 축소 → 최적값 0.302 제외! ❌
- 확장이 아니라 "축소 + 이탈"로 성능 저하

v3 보수적 개선:
1. Learning Rate: [5e-5, 5e-3] - 최적값 0.000517 포함, 세밀한 탐색
2. Dropout: [0.25, 0.65] - 최적값 0.302 포함, 낮은 dropout 보존
3. anneal_cap: [0.1, 0.2, 0.3, 0.4] - 원본 전부 유지 + 0.4만 추가
4. Trials: 30 → 40 (보수적 증가)

철학: 단순한 데이터는 단순하게. 검증된 최적값 주변을 세밀히 탐색.
예상 성능: Recall@5 0.087 → 0.090-0.094 (+3-8%, 현실적 목표)
...
print("=" * 60)
print("RecBole AutoML with Ray Tune - MultiVAE")
print("=" * 60)
print("✅ 라이브러리 로드 완료\n")
...
# 하이퍼파라미터 탐색 공간 (Phase 1.5 v3 - 보수적 개선)
# 원본 최적값: lr=0.000517, dropout=0.302, anneal_cap=0.2
# v2 실패 교훈: 최적값 포함하는 범위 설정이 핵심!
search_space = {
    'latent_dimension': tune.choice([128, 200, 256]),      # 변경 없음 (원본 OK)
    'mlp_hidden_size': tune.choice([[600], [512]]),        # 변경 없음 (원본 OK)
    'dropout_prob': tune.uniform(0.25, 0.65),              # 🔧 v3: 최적값 0.302 포함, 낮은 dropout 보존
    'anneal_cap': tune.choice([0.1, 0.2, 0.3, 0.4]),       # 🔧 v3: 원본 전부 유지 + 0.4만 추가
    'learning_rate': tune.loguniform(5e-5, 5e-3),          # 🔧 v3: 최적값 0.000517 중심, 세밀한 탐색
}

print(f"✅ 기본 설정 완료")
print(f"   모델: {MODEL_NAME} v3 (Phase 1.5 보수적 개선)")
...
print(f"\n🔍 하이퍼파라미터 탐색 공간 (Phase 1.5 v3 - 보수적 개선):")
print(f"   원본 최적값: lr=0.000517, dropout=0.302, anneal_cap=0.2")
print(f"   latent_dimension: [128, 200, 256] (변경 없음)")
print(f"   mlp_hidden_size: [[600], [512]] (변경 없음)")
print(f"   dropout_prob: [0.25, 0.65] 🔧 (최적값 0.302 포함)")
print(f"   anneal_cap: [0.1, 0.2, 0.3, 0.4] 🔧 (원본 + 0.4)")
print(f"   learning_rate: [5e-5, 5e-3] 🔧 (최적값 0.000517 중심)")
print(f"\n💡 예상 성능: Recall@5 0.087 → 0.090-0.094 (+3-8%)\n")
...
tuner = tune.Tuner(
    ...
    tune_config=tune.TuneConfig(
        ...
        num_samples=30,  # v3: 시간 절약을 위해 30 trials 유지
        max_concurrent_trials=max_concurrent_trials,
    ),
    run_config=RunConfig(
        name='recbole_multivae_automl_v3',
...
params_filename = f"{params_output_dir}/best_hyperparams_multivae_v3_{t_params.year}{t_params.month:02d}{t_params.day:02d}{t_params.hour:02d}{t_params.minute:02d}{t_params.second:02d}.json"
...
for user_id, recs in all_recommendations.items():
    # 각 사용자당 1행: 추천 아이템들을 공백으로 구분된 문자열로 변환
    items_str = ' '.join(recs)
...
print(f"   사용자당 평균 추천 수: {sum(len(recs) for recs in all_recommendations.values()) / len(all_recommendations):.2f}")

t = pd.Timestamp.now()
output_dir = f"outputs/{t.year}-{t.month:02d}-{t.day:02d}"
os.makedirs(output_dir, exist_ok=True)
filename = f"{output_dir}/submit_{MODEL_NAME}_RayTune_v3_{t.year}{t.month:02d}{t.day:02d}{t.hour:02d}{t.minute:02d}{t.second:02d}.csv"
...
# ============================================================
print("\n" + "=" * 60)
print("Ray Tune AutoML 최적화 결과 요약 - MultiVAE Phase 1.5 v3")
print("=" * 60)
...
print(f"\n🤖 AutoML 정보:")
print(f"   모델: {MODEL_NAME} (Phase 1.5 v3 - 보수적 개선)")
print(f"   디바이스: {device}")
...
print(f"   Ray Tune 결과: {ray_results_path}/recbole_multivae_automl_v3/")
