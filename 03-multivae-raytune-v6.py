#!/usr/bin/env python3
"""
RecBole AutoML - MultiVAE Hyperparameter Optimization with Ray Tune (v6 - Aggressive Tuning)
...
🔧 v6 개선사항 (목표: 0.092+):
1. Core-filtering 기준 완화: `min_interactions`를 5에서 3으로 변경하여 더 많은 학습 데이터를 확보합니다.
2. 모델 용량 및 탐색 공간 확장:
    - `mlp_hidden_size`: 더 깊고 넓은 MLP 구조([1024], [512, 256])를 탐색하여 모델 표현력을 강화합니다.
    - `anneal_cap`: 연속적인 탐색 공간(uniform)을 사용하여 KL-divergence 가중치를 더 세밀하게 조정합니다.
3. 탐색 횟수 대폭 증가: `num_samples`를 50에서 100으로 늘려 확장된 공간에서 최적점을 찾을 확률을 높입니다.
"""
...
print("=" * 60)
print("RecBole AutoML with Ray Tune - MultiVAE v6")
print("=" * 60)
print("✅ 라이브러리 로드 완료\n")
...
MODEL_NAME = 'MultiVAE'
MIN_INTERACTIONS = 3 # v6: 필터링 기준 완화
...
base_config = {
    ...
    # --- RecBole 내장 Core-filtering 적용 (v6 개선) ---
...
print(f"✅ 기본 설정 완료")
print(f"   모델: {MODEL_NAME} v6 (Aggressive Tuning)")
print(f"   타겟 메트릭: Recall@5")
print(f"   디바이스: {device}")
print(f"\n✨ v6 개선사항:")
print(f"   • RecBole Core-filtering: min_interactions={MIN_INTERACTIONS}")
print(f"   • 모델 용량 및 탐색 공간 확장")
print(f"   • 탐색 횟수 증가: 100회")
print(f"\n⚡ 성능 최적화 적용:")
...
tuner = tune.Tuner(
    ...
    tune_config=tune.TuneConfig(
        ...
        num_samples=100,  # ✨ v6: 탐색 횟수 증가
        max_concurrent_trials=max_concurrent_trials,
    ),
    run_config=RunConfig(
        name='recbole_multivae_automl_v6', # v6 버전으로 이름 변경
...
params_filename = f"{params_output_dir}/best_hyperparams_multivae_v6_{t_params.year}{t_params.month:02d}{t_params.day:02d}{t_params.hour:02d}{t_params.minute:02d}{t_params.second:02d}.json"
...
best_params_to_save = {
    ...
    'model': MODEL_NAME + "_v6", # v6 버전으로 모델 이름 변경
    'device': device,
...
filename = f"{output_dir}/submit_{MODEL_NAME}_v6_RayTune_{t.year}{t.month:02d}{t.day:02d}{t.hour:02d}{t.minute:02d}{t.second:02d}.csv" # v6 버전으로 파일명 변경
...
# ============================================================ 
print("\n" + "=" * 60)
print("Ray Tune AutoML 최적화 결과 요약 - MultiVAE v6")
print("=" * 60)
...
print(f"\n🤖 AutoML 정보:")
print(f"   모델: {MODEL_NAME}_v6") # v6 버전으로 모델 이름 변경
print(f"   디바이스: {device}")
...
print(f"   Ray Tune 결과: {ray_results_path}/recbole_multivae_automl_v6/") # v6 버전으로 경로 변경
