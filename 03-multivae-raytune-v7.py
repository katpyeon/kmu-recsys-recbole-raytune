#!/usr/bin/env python3
"""
RecBole AutoML - MultiVAE Hyperparameter Optimization with Ray Tune (v7 - Fine-grained Control)
...
🔧 v7 개선사항 (목표: 0.092+):
1. KL Annealing 스케줄 튜닝: `total_anneal_steps`를 탐색 공간에 추가하여 VAE의 핵심 학습 메커니즘을 최적화합니다.
2. 모델 용량 추가 확장: `latent_dimension`의 탐색 범위를 최대 512까지 늘려 모델의 표현력을 더욱 강화합니다.
3. v6 성공 전략 유지: `min_interactions=3`, 확장된 MLP 탐색 공간, `num_samples=100`을 그대로 유지합니다.
"""
...
print("=" * 60)
print("RecBole AutoML with Ray Tune - MultiVAE v7")
print("=" * 60)
print("✅ 라이브러리 로드 완료\n")
...
MODEL_NAME = 'MultiVAE'
MIN_INTERACTIONS = 3 # v6/v7: 필터링 기준 완화
...
base_config = {
    ...
    # --- RecBole 내장 Core-filtering 적용 (v6/v7) ---
...
# 하이퍼파라미터 탐색 공간 (v7, KL Annealing 및 Latent Dim 확장)
search_space = {
...
print(f"✅ 기본 설정 완료")
print(f"   모델: {MODEL_NAME} v7 (Fine-grained Control)")
print(f"   타겟 메트릭: Recall@5")
print(f"   디바이스: {device}")
print(f"\n✨ v7 개선사항:")
print(f"   • KL Annealing 스케줄 튜닝 추가")
print(f"   • Latent Dimension 탐색 공간 확장")
...
# 디바이스별 Trial 리소스 할당
if device == 'cuda':
    # CUDA: GPU 메모리 활용 최대화 (6개 동시 실행으로 전체 시간 단축)
...
tuner = tune.Tuner(
    ...
    tune_config=tune.TuneConfig(
        ...
        num_samples=100,  # ✨ v6/v7: 탐색 횟수 증가
        max_concurrent_trials=max_concurrent_trials,
    ),
    run_config=RunConfig(
        name='recbole_multivae_automl_v7', # v7 버전으로 이름 변경
...
params_filename = f"{params_output_dir}/best_hyperparams_multivae_v7_{t_params.year}{t_params.month:02d}{t_params.day:02d}{t_params.hour:02d}{t_params.minute:02d}{t_params.second:02d}.json"
...
best_params_to_save = {
    ...
    'model': MODEL_NAME + "_v7", # v7 버전으로 모델 이름 변경
    'device': device,
...
filename = f"{output_dir}/submit_{MODEL_NAME}_v7_RayTune_{t.year}{t.month:02d}{t_params.day:02d}{t.hour:02d}{t.minute:02d}{t.second:02d}.csv" # v7 버전으로 파일명 변경
...
# ============================================================ 
print("\n" + "=" * 60)
print("Ray Tune AutoML 최적화 결과 요약 - MultiVAE v7")
print("=" * 60)
...
print(f"\n🤖 AutoML 정보:")
print(f"   모델: {MODEL_NAME}_v7") # v7 버전으로 모델 이름 변경
print(f"   디바이스: {device}")
...
print(f"   Ray Tune 결과: {ray_results_path}/recbole_multivae_automl_v7/") # v7 버전으로 경로 변경
