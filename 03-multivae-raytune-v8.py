#!/usr/bin/env python3
"""
RecBole AutoML - MultiVAE Hyperparameter Optimization with Ray Tune (v8 - Final Tuning)
...
🔧 v8 개선사항 (목표: 0.092+):
1. 학습 시간 증가: `epochs`를 150으로, `stopping_step`을 20으로 늘려 모델의 충분한 수렴을 유도합니다.
2. 정규화 강화: `dropout_prob` 탐색 범위를 [0.5, 0.8]로 상향 조정하여 과적합을 방지합니다.
3. v7 전략 계승: KL Annealing 스케줄(`total_anneal_steps`) 튜닝과 확장된 `latent_dimension` 탐색을 유지합니다.
4. v6 성공 전략 유지: `min_interactions=3`, 확장된 MLP 탐색 공간, `num_samples=100`을 그대로 유지합니다.
"""
...
print("=" * 60)
print("RecBole AutoML with Ray Tune - MultiVAE v8")
print("=" * 60)
print("✅ 라이브러리 로드 완료\n")
...
MODEL_NAME = 'MultiVAE'
MIN_INTERACTIONS = 3 # v6/v7/v8: 필터링 기준 완화
...
base_config = {
    ...
    # --- RecBole 내장 Core-filtering 적용 (v6/v7/v8) ---
    ...
    'device': device,
    'epochs': 150,           # v8: 학습 시간 증가
    'stopping_step': 20,    # v8: Patience 증가
...
# 하이퍼파라미터 탐색 공간 (v8, 정규화 강화 및 학습 시간 연장)
search_space = {
    'latent_dimension': tune.choice([200, 256, 384, 512]),
    'mlp_hidden_size': tune.choice([[600], [512], [1024], [512, 256]]),
    'dropout_prob': tune.uniform(0.5, 0.8),
    'anneal_cap': tune.uniform(0.1, 0.5),
    'learning_rate': tune.loguniform(1e-4, 1e-2),
    'total_anneal_steps': tune.choice([20000, 60000, 100000, 200000]),
}

print(f"✅ 기본 설정 완료")
print(f"   모델: {MODEL_NAME} v8 (Final Tuning)")
print(f"   타겟 메트릭: Recall@5")
print(f"   디바이스: {device}")
print(f"\n✨ v8 개선사항:")
print(f"   • 학습 시간 증가: epochs=150, stopping_step=20")
print(f"   • 정규화 강화: dropout_prob [0.5, 0.8]")
print(f"   • v7 전략 계승: KL Annealing, Latent Dim 확장")
print(f"   • v6 전략 계승: min_interactions=3, 100회 탐색")
print(f"\n🔍 하이퍼파라미터 탐색 공간 (v8):")
print(f"   latent_dimension: [200, 256, 384, 512]")
print(f"   mlp_hidden_size: [[600], [512], [1024], [512, 256]]")
print(f"   dropout_prob: [0.5, 0.8] (uniform)")
print(f"   anneal_cap: [0.1, 0.5] (uniform)")
print(f"   learning_rate: [1e-4, 1e-2]")
print(f"   total_anneal_steps: [20000, 60000, 100000, 200000]\n")
...
scheduler = ASHAScheduler(
    metric='recall@5',
    mode='max',
    max_t=150, # v8: epochs 증가에 맞춰 max_t 증가
    grace_period=20, # v8: stopping_step 증가에 맞춰 grace_period 증가
    reduction_factor=2,
)
...
tuner = tune.Tuner(
    ...
    run_config=RunConfig(
        name='recbole_multivae_automl_v8', # v8 버전으로 이름 변경
...
params_filename = f"{params_output_dir}/best_hyperparams_multivae_v8_{t_params.year}{t_params.month:02d}{t_params.day:02d}{t_params.hour:02d}{t_params.minute:02d}{t_params.second:02d}.json"
...
best_params_to_save = {
    ...
    'model': MODEL_NAME + "_v8", # v8 버전으로 모델 이름 변경
    'device': device,
...
filename = f"{output_dir}/submit_{MODEL_NAME}_v8_RayTune_{t.year}{t.month:02d}{t_params.day:02d}{t.hour:02d}{t.minute:02d}{t.second:02d}.csv" # v8 버전으로 파일명 변경
...
# ============================================================ 
print("\n" + "=" * 60)
print("Ray Tune AutoML 최적화 결과 요약 - MultiVAE v8")
print("=" * 60)
...
print(f"\n🤖 AutoML 정보:")
print(f"   모델: {MODEL_NAME}_v8") # v8 버전으로 모델 이름 변경
print(f"   디바이스: {device}")
...
print(f"   Ray Tune 결과: {ray_results_path}/recbole_multivae_automl_v8/") # v8 버전으로 경로 변경

print("\n📊 상위 5개 Trial 결과:")
top5 = results_df.nlargest(5, 'recall@5')[['config/latent_dimension', 'config/mlp_hidden_size',
                                              'config/dropout_prob', 'config/anneal_cap',
                                              'config/learning_rate', 'config/total_anneal_steps',
                                              'recall@5', 'ndcg@5']]
