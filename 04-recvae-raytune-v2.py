#!/usr/bin/env python3
"""
RecBole AutoML - RecVAE Hyperparameter Optimization with Ray Tune (Phase 1 v2)

Phase 1 개선사항 (백화점 구매기록 데이터 최적화):
1. Learning Rate 확대: [1e-4, 1e-2] → [0.001, 0.01] (주요 개선점)
   - 이유: 기존 최적값 0.000517이 너무 작음 (10-20배 확대)
   - 백화점 구매 데이터: 희소성 높아 더 큰 learning rate 필요
2. Dropout 범위 조정: [0.3, 0.7] → [0.45, 0.65]
   - 이유: 최적값 0.52 주변에 집중, 과적합 방지
3. n_enc_epochs 강화: [1, 3] → [3, 5]
   - 이유: RecVAE는 인코더 사전학습이 핵심 (더 많은 반복)
4. 탐색 횟수 증가: 30 → 50 trials
   - 이유: 확대된 search space 충분히 탐색

예상 성능 향상: +20-30% (Recall@5: 0.087 → 0.104-0.113)
...
print("=" * 60)
print("RecBole AutoML with Ray Tune - RecVAE Phase 1 v2")
print("=" * 60)
print("✅ 라이브러리 로드 완료\n")
...
# ============================================================
print("=" * 60)
print("4. Ray Tune 설정 (Phase 1 v2)")
print("=" * 60)
...
# 하이퍼파라미터 탐색 공간 (Phase 1 v2 - 백화점 구매기록 최적화)
...
search_space = {
    ...
    'dropout_prob': tune.uniform(0.45, 0.65),              # Phase 1: 최적값 0.52 주변 집중
    'gamma': tune.loguniform(0.001, 0.01),                 # 적응형 β 계산용, 기본: 0.005
    'n_enc_epochs': tune.choice([3, 5]),                   # Phase 1: 인코더 사전학습 강화
    'learning_rate': tune.loguniform(0.001, 0.01),         # Phase 1: 10-20배 확대 (주요 개선점)
}
...
print(f"   dropout_prob: [0.45, 0.65] → 최적값 0.52 주변 집중")
print(f"   gamma: [0.001, 0.01] (적응형 β, 기본: 0.005)")
print(f"   n_enc_epochs: [3, 5] → 인코더 사전학습 강화 (RecVAE 핵심)")
print(f"   learning_rate: [0.001, 0.01] → 10-20배 확대 (주요 개선점)")
print(f"\n💡 RecVAE 특징: Composite Prior, 적응형 β, 교대 학습")
print(f"🎯 예상 성능 향상: +20-30% (Recall@5: 0.087 → 0.104-0.113)\n")
...
# ============================================================
print("=" * 60)
print("6. Ray Tune 하이퍼파라미터 최적화 시작 (Phase 1 v2)")
print("=" * 60)
...
tuner = tune.Tuner(
    ...
    tune_config=tune.TuneConfig(
        ...
        num_samples=50,  # Phase 1: 30 → 50 trials (확대된 search space 충분히 탐색)
        max_concurrent_trials=max_concurrent_trials,
    ),
    run_config=RunConfig(
        name='recbole_recvae_automl_v2',
...
params_filename = f"{params_output_dir}/best_hyperparams_recvae_v2_{t_params.year}{t_params.month:02d}{t_params.day:02d}{t_params.hour:02d}{t_params.minute:02d}{t_params.second:02d}.json"
...
filename = f"{output_dir}/submit_{MODEL_NAME}_RayTune_v2_{t.year}{t.month:02d}{t.day:02d}{t.hour:02d}{t.minute:02d}{t.second:02d}.csv"
...
# ============================================================
print("\n" + "=" * 60)
print("Ray Tune AutoML 최적화 결과 요약 - RecVAE Phase 1 v2")
print("=" * 60)
...
print(f"\n🤖 AutoML 정보:")
print(f"   모델: {MODEL_NAME} (Phase 1 v2)")
print(f"   디바이스: {device}")
...
print(f"   Ray Tune 결과: {ray_results_path}/recbole_recvae_automl_v2/")
