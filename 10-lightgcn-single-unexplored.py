#!/usr/bin/env python3
"""
LightGCN 단일 미탐색 파라미터 (메모리 효율)

Baseline: Recall@5 = 0.0882
한 번도 탐색하지 않은 파라미터 탐색 (1회 실행)
"""
...
search_space = {
    ...
    'embedding_size_extreme': tune.choice([64, 384, 512]),  # 극단값
}
...
tuner = tune.Tuner(
    ...
    tune_config=tune.TuneConfig(scheduler=scheduler, search_alg=search_alg, num_samples=20, max_concurrent_trials=1),
    run_config=RunConfig(name='lightgcn_single_unexplored', storage_path=ray_results_path),
)
...
print(f"\n📊 batch_size별 평균 성능:")
if 'config/train_batch_size' in successful_df.columns:
...
print(f"\n📊 embedding_size별 성능:")
if 'config/embedding_size_extreme' in successful_df.columns:
...
filename = f"{output_dir}/submit_LightGCN_SingleUnexplored_{t.year}{t.month:02d}{t.day:02d}{t.hour:02d}{t.minute:02d}{t.second:02d}.csv"
submission.to_csv(filename, index=False)

# 결과 저장
import json
result_file = f"{output_dir}/lightgcn_single_unexplored_{t.year}{t.month:02d}{t.day:02d}{t.hour:02d}{t.minute:02d}{t.second:02d}.json"
